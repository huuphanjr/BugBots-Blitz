...
public class TutorialRegulator extends Regulator
{
        ...
        BugBot2012 b;
	public TutorialRegulator(ArrayList<BugBot> b, int t)
	{
            super(b);
	...
	public void stateWinner()
	{
            if(tutorial == 1)
            {
                if(getGrid().get(new Location(0,5)) instanceof TutorialBugBot1)
                {
                    super.stateWinner();
	...
	public void act()
	{
            ...
            else if(tutorial == 3)
            {
                if(getStateWinner()==false)
                {
                    ...
                    if(getNumAct() == 0)
                    {
                        if(getGrid().get(new Location(0,0)) instanceof TutorialBugBot3)
                        {
                            PowerUp pU = new PowerUp(Color.BLUE);
                            rememberPULoc = new Location(0,3);
                            pU.putSelfInGrid(getGrid(),rememberPULoc);
                            
                            b = new BugBot2012(0,0,5,"The Enemy", Color.RED);
                            b.putSelfInGrid(getGrid(), new Location(3,0));
                        }
                        else if(getGrid().get(new Location(0,6)) instanceof TutorialBugBot3)
                        {
                            PowerUp pU = new PowerUp(Color.BLUE);
                            rememberPULoc = new Location(0,3);
                            pU.putSelfInGrid(getGrid(),rememberPULoc);
                        ...
                        else if(getGrid().get(new Location(6,0)) instanceof TutorialBugBot3)
                        {
                            PowerUp pU = new PowerUp(Color.BLUE);
