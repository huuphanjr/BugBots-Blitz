...
public class phanfromvietnam extends BugBot2012
{
...
//     Case1: Quadrant I:
   ...
   //Checking if the bug is valid in new grid so the bug dont have to avoid the Rock failing
    ...
    public Location chasingEnemy(ArrayList<BugBot>enemies){
...
//                    }
    ...
    public Location getDangerousEnemy(ArrayList<BugBot>enemies){
        Location bugLoc=getLocation();
        int bugDir=getDirection();
        Location output=null;
        if(enemies.size()==0){
...
//    }
    ...
    public Location scanning(ArrayList<BugBot> enemies){
        Location output=null;
        runAwayMode=false;
        if(enemies.size()!=0){
            for(BugBot enemy:enemies){
                if(enemy.getStrength()-getDefense()>=3&&((BugBot2012)enemy).getAmmo()>0){
                    runAwayMode=true;
                    output=enemy.getLocation();
                }
            }
    ...
    {  
        ...
        ArrayList<BugBot> enemies=new ArrayList<>();
        for(int i=0;i<actors.size();i++){
           Actor curr=actors.get(i);
           if(curr instanceof BugBot){
               enemies.add((BugBot)curr);
           }            
        }
        //======================================================================
