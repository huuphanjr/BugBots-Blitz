 ...
 * @author emsuri
 */
public class BugBot2012 extends BugBot
{
    private int ammo;
    ...
    public BugBot2012(int s, int d, int sp, String n, Color co)
    {
        super(s,d,sp,n,co);
        ammo = 2*s;
        killCount=0;
    }
    public BugBot2012(int s, int d, int sp, String n, Color co, String kS, String vM)
    {
        super(s,d,sp,n,co);
        ammo = 2*s;
        killCount=0;
    ...
    public void attack()
    {
        if(ammo > 0)
        {
            ...
            for (Location a : locs) {
                if (getGrid().isValid(a) && this.getGrid().get(a) != null && this.getGrid().get(a) instanceof BugBot &&((BugBot) (this.getGrid().get(a))).getDefense() <= getStrength() - 3)
                {
                    destroy(((BugBot) (this.getGrid().get(a))));
                    if(killSong!=null)
                    {
                ...
                else if(getGrid().isValid(a) &&!(this.getGrid().get(a) instanceof BugBot)&& !(this.getGrid().get(a) instanceof Rock)&& !(this.getGrid().get(a) instanceof Regulator))
                {
